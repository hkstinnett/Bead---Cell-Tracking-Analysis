...
for (i in 1:(length(cells.fgf8))){
  sub.fgf8[[i]]<-data.frame(X=(cells.fgf8[[i]]$X-rep(bead.fgf8[[i]]$X,30)),
                            Y=(cells.fgf8[[i]]$Y-rep(bead.fgf8[[i]]$Y,30)),
                            Time=cells.fgf8[[i]]$Slice,
                            Track=cells.fgf8[[i]]$Track)
}

for (i in 1:(length(cells.ctrl))){
  sub.ctrl[[i]]<-data.frame(X=(cells.ctrl[[i]]$X-rep(bead.ctrl[[i]]$X,30)),
                            Y=(cells.ctrl[[i]]$Y-rep(bead.ctrl[[i]]$Y,30)),
...
dim(Distance.fgf8)<-c(100,30,(length(sub.fgf8)))

Distance.ctrl<-sapply(sub.ctrl, 
                      function (embryo) by(embryo, embryo$Track, 
                      function(embryo.track) sqrt(embryo.track$X^2+embryo.track$Y^2)))
Distance.ctrl<-unlist(Distance.ctrl)
dim(Distance.ctrl)<-c(100,30,(length(sub.ctrl)))
...
plot(1:100, Distance.fgf8.mean, type="l",col="maroon",
     ...
     xlab="Time/8 min",ylab="average bead-cell distance (µm)", 
     ylim = c(72, 130) )
lines(1:100, Distance.ctrl.mean, col="black")

#I want to plot each individual distance, not just mean.

plot(1:100, Distance.fgf8[1:5], type="l", col="maroon",xlab="Time/8 min",ylab="average bead-cell distance (µm)")
